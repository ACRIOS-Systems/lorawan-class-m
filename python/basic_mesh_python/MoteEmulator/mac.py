...
class Gateway:
    ...
    def pull(self, transmitter):
        ...
        print(int.from_bytes(res[0][3:4], "big"))
        if(int.from_bytes(res[0][3:4], "big") == 0x04):
            self.parse_pullack(res[0])
        elif (int.from_bytes(res[0][3:4], "big") == 3):
            return res

    def TX_ACK(self, gatewayEUI, resp, transmitter):
        TX_ACK = bytearray()
        TX_ACK.append(2)
        TX_ACK.append(int.from_bytes(resp[0][1:2], "big"))
        TX_ACK.append(int.from_bytes(resp[0][2:3], "big"))
        TX_ACK.append(0x05)
        gatewayEUI = bytes.fromhex(gatewayEUI)
        TX_ACK = TX_ACK+gatewayEUI+b'{"txpk_ack":{"error":"NONE"}}'
        print(TX_ACK)
        transmitter.send(TX_ACK)
    ...
    def push(self, transmitter, data, mote):
        ...
        try:
            ...
            return self.parse_pullresp(pullresp[0], mote, False)
    ...
    def parse_pullresp(self, pullresp, mote, return_phypld:bool):
        """
        ...
        return self.parse_txpk(txpk, mote, return_phypld)

    def parse_txpk(self, txpk, mote, return_phypld:bool):
        """
        ...
        if return_phypld != True:
            return mote.parse_phypld(phypld)
        else:
            return phypld

    
    def parseMHDR(self, mhdr):
        """Parse the MHDR payload of the received data packet"""
        name = ('mtype', 'rfu', 'major')
        bitlength = (3, 3, 2)
        offset = (5, 2, 0)
        assert len(name) == len(offset) == len(bitlength)
        mhdr = int.from_bytes(mhdr, byteorder='little')
        ...
        for ind, value in enumerate(name):
            ...
            res[ind] = (mhdr & mask) >> off
        return res
    

    def parseMACPLD(self, phypld):
        """Parse the MAC payload of the received data packet and return the device address"""
        ...
        devaddr = devaddr.hex()
        devaddr = bytearray.fromhex(devaddr)
        devaddr.reverse()
        return devaddr.hex()

    def extractDeviceAddressAndMtype(self, phypld):
        phypld = memoryview(phypld)
        mhdr = phypld[:MHDR_LEN]
        mtype, _, major = self.parseMHDR(mhdr)
        if mtype == 3 or mtype == 5:
            deviceAddress = self.parseMACPLD(phypld)
            return (mtype, deviceAddress)
        return False
...
class Mote:
    ...
    def __init__(self, joineui, deveui, appkey, nwkkey, dbpath='MoteEmulator/models', **kwargs):
    ...
    def form_fhdr(self, fopts, version='1.0.2', ack=False):
        """
        Form FHDR field
        Args:
    ...
    def calcmic_app(self, mhdr, fhdr, ack=False, fport=None, frmpld=None, direction=0, fcnt=0):
        ...
        if direction == 0:
            ...
            if self.version != '1.1':
                conffcnt = 0 # ConfFCnt is zero in 1.0 version
        B0_elements = [
            b'\x49',
            conffcnt,
            0,
        ...
        if direction == 0 and self.version == '1.1': # Only uplink message has B1 (version 1.1)
    ...
    def parse_joinacpt(self, mhdr, joinacptmic):
        ...
        if (cmic == mic):
            ...
            parsedPacket = {"MType" : 1,
                            "DevAddr": self.devaddr.hex(),
                            "JoinNonce": self.joinnonce.hex(),
                            "NetID": self.homenetid.hex(),
                            "optneg": optneg,
                            "rx1droffset": self.rx1droffset,
                            "rx2dr": self.rx2dr,
                            "MIC": cmic.hex()}
            return (parsedPacket)
        else:
    ...
    def parse_phypld(self, phypld):
        ...
        if mtype == 1:
            ...
            return self.parse_joinacpt(mhdr, macpldmic)
        else:  # PULL RESP for app phypayload
            macpld = phypld[MHDR_LEN:-MIC_LEN]
            mic = phypld[-MIC_LEN:]
            return self.parse_macpld(mtype, mhdr, macpld, mic)
    ...
    def parse_macpld(self, mtype, mhdr, macpld, mic):
        ...
        if (cmic == mic):
            ...
            if confirmed:
                ...
                prefix = ''
            message_type = prefix + "uplink/downlink"
            ...
            parsedPacket = {"MType" : mtype,
                            "EndDeviceAddr": devaddr.hex(),
                            "FPort": fport,
                            "FCntDown": fcntdown,
                            "Payload": frmpld
                            }
