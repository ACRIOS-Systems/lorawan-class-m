...
def networkServerGetNodeKeys(deveui):
    return ns.getDeviceAppKey(deveui)

def listProvisionedGateways():
    return ns.listGateways()

def listProvisionedDevices():
    return ns.listDevices()

def networkServerPurge():
    for dev in ns.listDevices():
        if dev.device_profile_name==MESHDeviceProfileName:
            networkServerUnregisterNode(dev.dev_eui)

    for gw in ns.listGateways():
        if gw.organization_id==ns.getOrganizationByName(MESHOrganizationName).id:
            networkServerUnregisterGateway(gw.name)

    #for devProfile in ns.listDeviceProfiles():
    #    pass
    #    ns.unregisterDevice(devProfile.)
    #
    #for gwProfile in ns.listGatewayProfiles():
    #    pass
    #    ns.deleteGateway(gwProfile.)
    #
    #for svcProfile in ns.listServiceProfiles():
    #    pass
