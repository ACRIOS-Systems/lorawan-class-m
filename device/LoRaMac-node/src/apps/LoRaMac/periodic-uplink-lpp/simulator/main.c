...
static int setKeys(uint8_t* DevEui, uint8_t* AppEui, uint8_t* AppKey, int joinDR);
...
int hextobin(char* hex, uint8_t* bin, int reqLen)
{
    if(strlen(optarg) != ((reqLen*3)-1)) // format is "AA:BB:.....:CC"
    {
        return -1; // nonono
    }
...
int main( int argc, char* argv[] )
{
    ...
    uint8_t AppKey[16];
    int joinDR;
    uint8_t DevEuiSet = false;
    uint8_t AppEuiSet = false;
    uint8_t AppKeySet = false;
    int joinDRSet = false;
    int opt;

    while((opt = getopt(argc, argv, "d:a:k:j:")) != -1)
    {
        switch (opt)
        {
        case 'd':
            if(0 != hextobin(optarg, DevEui, 8))
            {
                printf("Bad DevEUI, use format: 'AA:BB:CC:DD:EE:FF:00:11'\n");
                return -1;
            }
            DevEuiSet = true;
            break;
        case 'a':
            if(0 != hextobin(optarg, AppEui, 8))
            {
                printf("Bad AppEui, use format: 'AA:BB:CC:DD:EE:FF:00:11'\n");
                return -1;
            }
            AppEuiSet = true;
            break;
        case 'k':
            if(0 != hextobin(optarg, AppKey, 16))
            {
                printf("Bad AppKey, use format: 'AA:BB:CC:DD:EE:FF:00:11:AA:BB:CC:DD:EE:FF:00:11'\n");
                return -1;
            }
            AppKeySet = true;
            break;
        case 'j':
            sscanf(optarg, "%d", &joinDR);
            if((joinDR < 0)||(joinDR > 5))
            {
                printf("Value of 0 to 5 is allowed for join datarate (reminder: DR0 == SF12 and DR5 == SF7)\n");
                return -1;
            }
            joinDRSet = true;
            break;
    ...
    if(!(joinDRSet && AppKeySet && AppEuiSet && DevEuiSet))
    {
        printf("Please set all parameters! (-a, -d, -k, -j) \n");
        return -1;
    }
    else
    {
    ...
    setKeys(DevEui, AppEui, AppKey, joinDR);
...
static int setKeys(uint8_t* DevEui, uint8_t* AppEui, uint8_t* AppKey, int joinDR)
{
    ...
    LoRaMacStatus_t ret = LoRaMacMibSetRequestConfirm( &mibReq );
    ...
    // copy data
      {
        SecureElementSetKey(APP_KEY, AppKey);
        SecureElementSetKey(APP_S_KEY, AppKey);
        SecureElementSetKey(J_S_INT_KEY, AppKey);
        SecureElementSetKey(J_S_ENC_KEY, AppKey);
        SecureElementSetKey(NWK_KEY, AppKey); //<--- OTAA: used for joining



        SecureElementSetKey(S_NWK_S_INT_KEY, AppKey);
        SecureElementSetKey(NWK_S_ENC_KEY, AppKey);
        SecureElementSetKey(NWK_S_ENC_KEY, AppKey);
        SecureElementSetKey(F_NWK_S_INT_KEY, AppKey);
        SecureElementSetDevEui(DevEui);
      }


    // set DR to be used during join
    LmHandlerParams.TxDatarate = joinDR;

   if(1) //(LoRaWAN_getOTAAState())
   {
