# MAIN CONSTANTS
SAMPLING_FREQUENCY = 1e6
T_OFFSET = 500
FFT_FACTOR = 8
CARRIER_SIZE = 1024
IS_LORAWAN = True


# VALID SPREADING FACTORS
SF_7 = 7
SF_8 = 8
SF_9 = 9
SF_10 = 10
SF_11 = 11
SF_12 = 12

VALID_SFS = [SF_7, SF_8, SF_9, SF_10, SF_11, SF_12]

# SAMPLES PER SYMBOL
# TODO: recalculate based on VALID_SFS
SF_7_SAMPLES_N = "SF7_Samples"
SF_8_SAMPLES_N = "SF8_Samples"
SF_9_SAMPLES_N = "SF9_Samples"
SF_10_SAMPLES_N = "SF10_Samples"
SF_11_SAMPLES_N = "SF11_Samples"
SF_12_SAMPLES_N = "SF12_Samples"

SF_SAMPLES_N = [SF_7_SAMPLES_N, SF_8_SAMPLES_N, SF_9_SAMPLES_N, SF_10_SAMPLES_N, SF_11_SAMPLES_N, SF_12_SAMPLES_N]

# VALID BANDWIDTHS
BW_1 = 125e3
BW_2 = 250e3
BW_3 = 500e3
VALIDS_BWS = [BW_1, BW_2, BW_3]

# DEMODULATOR CONSTANTS
CARRIER_OFFSET = 1024
MINIMUM_SPAN_SAMPLES = 50
SPAN_COEFFICIENT = 0.25
PERIOD_SPAN = 0.30
PERIOD_VARIANCE = 0.01
SF_THRESHOLD_SKIP = 0.10
SF_THRESHOLD_SKIP_END = 0.75
SF_THRESHOLD_COEFFICIENT = 1.5
INITIAL_PEAK_SPAN = 0.25
INITIAL_CORRELATION_COEFFICIENT = 0.50
FFT_DISTANCE = 0.05
FFT_FACTOR = 8
TIME_OFFSET = 500
RECEIVE_THRESHOLD = 4
DOWNSAMPLING_FREQUENCIES = [5e5, 7.5e5, 1.5e6]
BW_THRESHOLD = 0.33


# EXPLICIT HEADER CONFIG #
EXPLICIT_HEADER_LEN = 8
EXPLICIT_HEADER_CODEWORDS = 5
EXPLICIT_HEADER_CR = 4

# VALID CODING RATES
CR_5_8 = 1
CR_6_8 = 2
CR_7_8 = 3
CR_8_8 = 4

VALID_CRS = [CR_5_8, CR_6_8, CR_7_8, CR_8_8]

# DECODER CONSTANTS
LOW_NIBBLE_MASK = 0x0F
HIGH_NIBBLE_MASK = 0xF0
LDR_MS = 16

# KEYS
KEYS = {
    '019C4B8E' : {
        "nwsKey" : "A54F66E173B919CA6BA8375C9B2D7106",
        "appsKey" : "A54F66E173B919CA6BA8375C9B2D7106"
    },
    '01C5691A' : {
        "nwsKey" : "BA0F8D35E543816DDD71CD622B11D7D6",
        "appsKey" : "1EB1D60960D9B2EFB9F3103BACC975D9"
    }
    
}

# Whitening sequence
W_SEQ = [
            
]