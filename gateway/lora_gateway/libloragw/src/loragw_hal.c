...
#include "loragw_debug.h" /* debug stuff */
...
#define DEBUG_MSG(str)                do {if (debug_flags.hal) fprintf(stderr, str);} while(0)
#define DEBUG_PRINTF(fmt, args...)    do {if (debug_flags.hal) fprintf(stderr,"%s:%d: "fmt, __FUNCTION__, __LINE__, args);} while(0)
#define DEBUG_ARRAY(a,b,c)            do {if (debug_flags.hal) {\
                                            for(a=0;a<b;++a) \
                                                fprintf(stderr,"%x.",c[a]);\
                                            fprintf(stderr,"end\n");}\
                                        } while(0)
#define CHECK_NULL(a)                 do {if (a==NULL) {\
                                            if(debug_flags.hal){\
...
struct rx_params_struct {
    uint32_t frequency;
    uint8_t preamble_length;
    uint8_t header_implicit; // 0 = explicit header, 1 = implicit header
    uint32_t size;
    uint8_t crc_status;         // 0 - no CRC, 1 - CRC valid, 2 - CRC invalid
    uint32_t crc;
    uint8_t iq_inverted;        // 0 - normal, 1 - inverted
    uint8_t spreading_factor;
    uint32_t bandwidth_kHz;
    uint8_t coding_rate;
    uint8_t low_data_rate;  // 0 - normal, 1 - low data rate
    int32_t snr;
    int32_t rssi;
};

typedef struct rx_params_struct rx_params_st;


pthread_t stdinThread;
pthread_fifo_t stdinQueue;

// process stdin in separate thread
void* stdinThreadFunction(void* arg)
{
    ...
    // queue for stdin lines is given in arg
    ...
    while(1)
    {
        ...
        if (c == '\n')
        {
            ...
            // put line into queue
            status = pthread_fifo_enqueue(queue, (uint8_t*)linebuf);

            if (status) {
                fprintf(stderr, "ERROR: failed to enqueue stdin line err=%d\n", status);
                continue;
            }
        }
    }
...
void ParseCmd(const char *cmd, uint8_t *buff, uint8_t *dbuf)
{
    char* parameter;
    ...
    // zero the whole rx_par structure
    memset(&rx_par, 0, sizeof(rx_par));
    memset(metadata, 0, sizeof(metadata));
    
    // parse cmd - most is adopted from LoRaMac-node/src/boards/simulator/uart-board.c
    // Examples:
    // RX_DONE, spreading_factor=12, coding_rate=5, size=23, data=00:00:00:00:00:00:00:00:00:00:00:00:00:00:00:00:00:07:00:D9:BB:81:93:
    // :RX_DONE, ts=3, frequency=868500000, preamble_length=8, header_implicit=0, crc_status=1, iq_inverted=0, spreading_factor=12, bandwidth_kHz=125, coding_rate=5, size=23, data=00:01:00:FF:EE:DD:CC:BB:AA:02:00:FF:EE:DD:CC:BB:AA:01:00:D9:83:00:0B:
    // :TX,ts=0,frequency=868100000,preamble_length=8,header_type_explicit=1,payload_length=23,crc_mode_on=1,iq_normal=1,spreading_factor=12,bandwidth_kHz=125,coding_rate=4_5,low_data_rate=1,power=13,ramp_time=40,tx_timeout=0,symbol_timeout=0,size=23,data=00:00:00:00:00:00:00:00:00:E4:8D:11:8A:C9:D1:93:95:01:00:CE:46:EF:37:


    parameter = strstr(cmd, "size=");
    if(parameter)
    {
        int tmpi;
        sscanf(parameter, "size=%d", &tmpi);
        rx_par.size = (uint32_t)tmpi;
    }
    else
    {
        // abort!
    ...
    {
        // abort!
        fprintf(stderr, ":!MISSING 'data=xx:xx:...'\n");
        return;
    }

    parameter = strstr(cmd, "frequency=");
    if(parameter)
    {
        int tmpi;
        sscanf(parameter, "frequency=%d", &tmpi);
        rx_par.frequency = (uint32_t)tmpi;
    }

    parameter = strstr(cmd, "preamble_length=");
    if(parameter)
    {
        int tmpi;
        sscanf(parameter, "preamble_length=%d", &tmpi);
        rx_par.preamble_length = (uint8_t)tmpi;
    }

    parameter = strstr(cmd, "header_type_implicit=");
    if(parameter)
    {
        int tmpi;
        sscanf(parameter, "header_implicit=%d", &tmpi);
        rx_par.header_implicit = (uint8_t)tmpi;
    }
   
    parameter = strstr(cmd, "crc_status=");
    if(parameter)
    {
        int tmpi;
        sscanf(parameter, "crc_status=%d", &tmpi);
        rx_par.crc_status = (uint8_t)tmpi;
    }

    parameter = strstr(cmd, "crc=");
    if(parameter)
    {
        int tmpi;
        sscanf(parameter, "crc=%d", &tmpi);
        rx_par.crc = (uint32_t)tmpi;
    }    

    parameter = strstr(cmd, "iq_inverted=");
    if(parameter)
    {
        int tmpi;
        sscanf(parameter, "iq_inverted=%d", &tmpi);
        rx_par.iq_inverted = (uint8_t)tmpi;
    }

    parameter = strstr(cmd, "spreading_factor=");
    if(parameter)
    {
        int tmpi;
        sscanf(parameter, "spreading_factor=%d", &tmpi);
        rx_par.spreading_factor = (uint8_t)tmpi;
    }

    parameter = strstr(cmd, "bandwidth_kHz=");
    if(parameter)
    {
        int tmpi;
        sscanf(parameter, "bandwidth_kHz=%d", &tmpi);
        rx_par.bandwidth_kHz = (uint32_t)tmpi;
    }

    parameter = strstr(cmd, "coding_rate=");
    if(parameter)
    {
        int tmpi;
        sscanf(parameter, "coding_rate=%d", &tmpi);
        rx_par.coding_rate = (uint8_t)tmpi;
    }

    parameter = strstr(cmd, "low_data_rate=");
    if(parameter)
    {
        int tmpi;
        sscanf(parameter, "low_data_rate=%d", &tmpi);
        rx_par.low_data_rate = (uint8_t)tmpi;
    }
    ...
    if(parameter)
    {
        ...
        rx_par.rssi = (int32_t)tmpi;
    }

    //
    // fill the output buff with the rx parameters and payload
    //
    ...
    */
    ...
    buff[0] = 1; // number of packets available in RX data buffer

    // buff[3]
    switch(rx_par.crc_status)
    {
        case 0:
            buff[3] = 1; // STAT_NO_CRC;
            break;
        case 1:
            buff[3] = 5; // STAT_CRC_OK;
            break;
        case 2:
            buff[3] = 7; // STAT_CRC_BAD;
            break;
    }
    ...
    // copy payload to dbuf
    memcpy(dbuf, rx_payload, rx_par.size);

    //
    // fill metadata which will follow the payload
    //
    ...
    int if_chain = 0;
    bool found = false;
    for(if_chain = 0; if_chain < LGW_IF_CHAIN_NB; if_chain++)
    {
        uint8_t rf_chain = (uint8_t)if_rf_chain[if_chain];
        uint32_t f = (uint32_t)((int32_t)rf_rx_freq[rf_chain] + if_freq[if_chain]);
        if (f == rx_par.frequency)
        {
            found = true;
            break;
        }
    ...
    if(!found)
    {
        fprintf(stderr, ":!ERROR: IF chain not found for frequency %d\n", rx_par.frequency);
        return;
    }

    metadata[0] = if_chain; // if_chain

    // rssi
    metadata[5] = 0; // rssi: p->rssi = (float)buff[sz+5] + rf_rssi_offset[p->rf_chain];
    
    // snr
    metadata[2] = 0; // snr: p->snr = ((float)((int8_t)buff[sz+2]))/4;
    metadata[3] = 0; // snr_min: p->snr_min = ((float)((int8_t)buff[sz+3]))/4;
    metadata[4] = 0; // snr_max: p->snr_max = ((float)((int8_t)buff[sz+4]))/4;

    // spreading factor
    metadata[1] = rx_par.spreading_factor << 4;

    // coding rate
    metadata[1] |= (rx_par.coding_rate - 4) << 1;

    // timestamp
    struct timeval current_unix_time;
    gettimeofday(&current_unix_time, NULL);
    uint32_t raw_timestamp = (uint32_t)(current_unix_time.tv_sec * 1000000UL + current_unix_time.tv_usec);
    metadata[6] = (raw_timestamp >> 0) & 0xFF;
    metadata[7] = (raw_timestamp >> 8) & 0xFF;
    metadata[8] = (raw_timestamp >> 16) & 0xFF;
    metadata[9] = (raw_timestamp >> 24) & 0xFF;

    printf("raw_timestamp: %u : %02X %02X %02X %02X \n", raw_timestamp, metadata[6], metadata[7], metadata[8], metadata[9]);
    ...
    */
    ...
    // copy metadata to dbuf
    memcpy(&dbuf[rx_par.size], metadata, 12);
...
#ifdef SIMULATOR
    fprintf(stderr, ":CFG_RXRF,rf_chain=%d,en=%d,freq=%d,rssi_offset=%f,radio_type=%d,tx_enable=%d,tx_notch_freq=%u\n", rf_chain, rf_enable[rf_chain], rf_rx_freq[rf_chain], rf_rssi_offset[rf_chain], rf_radio_type[rf_chain], rf_tx_enable[rf_chain], rf_tx_notch_freq[rf_chain]);
#endif /* SIMULATOR */
...
#ifdef SIMULATOR
            fprintf(stderr, ":CFG_RXIF,rf_chain=%d,type=lora_std,if_chain=%d,en=%d,freq=%d,bw=%d,SF_mask=0x%02x\n", if_rf_chain[if_chain], if_chain, if_enable[if_chain], if_freq[if_chain], lgw_bw_getval(conf.bandwidth), lora_rx_sf);
#endif /* SIMULATOR */
            break;
...
#ifdef SIMULATOR
            fprintf(stderr, ":CFG_RXIF,rf_chain=%d,type=lora_multi,if_chain=%d,en=%d,freq=%d,bw=%d,SF_mask=0x%02x\n", if_rf_chain[if_chain], if_chain, if_enable[if_chain], if_freq[if_chain], lgw_bw_getval(conf.bandwidth), lora_multi_sfmask[if_chain]);
#endif /* SIMULATOR */
            break;
...
//
//#ifdef SIMULATOR
//            fprintf(stderr, ":CFG_RXIF: FSK if_chain %d configuration; en:%d freq:%d bw:%d dr:%d (%d real dr) sync:0x%0*llX\n", if_chain, if_enable[if_chain], if_freq[if_chain], lgw_bw_getval(fsk_rx_bw), fsk_rx_dr, LGW_XTAL_FREQU/(LGW_XTAL_FREQU/fsk_rx_dr), 2*fsk_sync_word_size, fsk_sync_word);
//#endif /* SIMULATOR */
...
int lgw_txgain_setconf(struct lgw_tx_gain_lut_s *conf) {
    ...
    for (i = 0; i < txgain_lut.size; i++) {
        ...
        if (conf->lut[i].rf_power > 27) {
            DEBUG_MSG("ERROR: TX gain LUT: RF Power gain must not exceed 27 dBm\n");
            return LGW_HAL_ERROR;
        }
...
int lgw_start(const char *spi_dev_path, int spi_speed) {
    int i, err;
    int reg_stat;
    unsigned x;
    ...
    status = pthread_fifo_create(&stdinQueue, 64, LINE_MAX_LEN);
	if (status != 0) {
		fprintf(stderr, "Failed to create queue\n");
		exit(-1);
	}

    status = pthread_create( &stdinThread, NULL, stdinThreadFunction, &stdinQueue);
    if (status != 0) {
    ...
    reg_stat = lgw_connect(false, rf_tx_notch_freq[rf_tx_enable[1]?1:0], spi_dev_path, spi_speed);
    ...
    if (err != 0) {
        printf("ERROR: Failed to setup sx125x radio for RF chain 0\n");
        return LGW_HAL_ERROR;
    }
    ...
    if (lbt_is_enabled() == true) {
        ...
        if (i != LGW_LBT_SUCCESS) {
            printf("ERROR: lbt_setup() did not return SUCCESS\n");
            return LGW_HAL_ERROR;
        }
        ...
        i = lbt_start();
        if (i != LGW_LBT_SUCCESS) {
            printf("ERROR: lbt_start() did not return SUCCESS\n");
            return LGW_HAL_ERROR;
        }
    }
    ...
    */
    ...
    switch (rf_radio_type[0]) { /* we assume that there is only one radio type on the board */
        ...
        case LGW_RADIO_TYPE_SX1257:
            ...
            break;
        default:
            printf("ERROR: UNEXPECTED VALUE %d FOR RADIO TYPE\n", rf_radio_type[0]);
            break;
    }
    ...
    */
    if ((cal_status & 0x81) != 0x81) {
        printf("ERROR: CALIBRATION FAILURE (STATUS = %u)\n", cal_status);
        return LGW_HAL_ERROR;
    } else {
        printf("Note: calibration finished (status = %u)\n", cal_status);
    }
    ...
    if (rf_enable[0] && rf_tx_enable[0] && ((cal_status & 0x20) == 0)) {
        printf("WARNING: problem in calibration of radio A for TX DC offset\n");
    }
    if (rf_enable[1] && rf_tx_enable[1] && ((cal_status & 0x40) == 0)) {
        printf("WARNING: problem in calibration of radio B for TX DC offset\n");
    }
    ...
    if (rf_rx_freq[0] == 0) {
        printf("ERROR: wrong configuration, rf_rx_freq[0] is not set\n");
        return LGW_HAL_ERROR;
    }
    ...
    */
    ...
    if (fw_version != FW_VERSION_AGC) {
        printf("ERROR: Version of AGC firmware not expected, actual:%d expected:%d\n", fw_version, FW_VERSION_AGC);
        return LGW_HAL_ERROR;
    }
    ...
    if (fw_version != FW_VERSION_ARB) {
        printf("ERROR: Version of arbiter firmware not expected, actual:%d expected:%d\n", fw_version, FW_VERSION_ARB);
        return LGW_HAL_ERROR;
    }
    ...
    lgw_reg_r(LGW_MCU_AGC_STATUS, &read_val);
    if (read_val != 0x10) {
        printf("ERROR: AGC FIRMWARE INITIALIZATION FAILURE, STATUS 0x%02X\n", (uint8_t)read_val);
        return LGW_HAL_ERROR;
    }
...
int lgw_receive(uint8_t max_pkt, struct lgw_pkt_rx_s *pkt_data) {
    ...
    for (nb_pkt_fetch = 0; nb_pkt_fetch < max_pkt; ++nb_pkt_fetch) {
        ...
        uint8_t dbuf[255+RX_METADATA_NB]; /* buffer to store simulator command processing */

        // TODO: receive packet from simulator here
        //printf("==================================== IN RECEIVE ====================================\n");
        char bufline[LINE_MAX_LEN];
        struct timespec timeout = {0, 10000}; /* 10us */
        int status = pthread_fifo_dequeue(&stdinQueue, (uint8_t*)&bufline, &timeout);
        if (status == 0) {
            printf("Received from stdin queue: %s\n", bufline);

            // parse the command
            memset(dbuf, 0, sizeof(dbuf));
            ParseCmd(bufline, buff, dbuf);
        } else {
            buff[0] = 0;
        }
        ...
        // TODO: receive packet from simulator here
#ifdef SIMULATOR      
        // copy dbuf data to buff
        memcpy(buff, dbuf, sizeof(buff));
#else
        ...
        printf("raw_timestamp: %u, timestamp_correction: %u, count_us: %u\n", raw_timestamp, timestamp_correction, p->count_us);
...
int lgw_send(struct lgw_pkt_tx_s pkt_data) {
    ...
    sprintf(cmd, ":TX modulation=%s,", (pkt_data.modulation == MOD_LORA) ? "LORA" : "FSK");
    sprintf(cmd, "%s frequency=%u,", cmd, pkt_data.freq_hz);
    sprintf(cmd, "%s power=%d,", cmd, pkt_data.rf_power);
    sprintf(cmd, "%s size=%u,", cmd, pkt_data.size);
    sprintf(cmd, "%s bandwidth_kHz=%u", cmd, (1000 >> pkt_data.bandwidth));     // BW_500KHZ = 1, BW_250KHZ = 2, BW_125KHZ = 3 ...
    sprintf(cmd, "%s rf_chain=%u,", cmd, pkt_data.rf_chain);
    sprintf(cmd, "%s crc_en=%u,", cmd, (pkt_data.no_crc ? 0 : 1));
    sprintf(cmd, "%s preamble_length=%u,", cmd, pkt_data.preamble);
   
    if (pkt_data.tx_mode == TIMESTAMPED)
        pkt_data.count_us;      //  timestamp to transmit the packet
    ...
    if (pkt_data.modulation == MOD_LORA) {
        ...
        switch (pkt_data.datarate) {
            case DR_LORA_SF7:  spreading_factor = 7; break;
            case DR_LORA_SF8:  spreading_factor = 8; break;
            case DR_LORA_SF9:  spreading_factor = 9; break;
            case DR_LORA_SF10: spreading_factor = 10; break;
            case DR_LORA_SF11: spreading_factor = 11; break;
            case DR_LORA_SF12: spreading_factor = 12; break;
        }
        int coderate = 0;
        switch (pkt_data.coderate) {
            case CR_LORA_4_5: coderate = 5; break;
            case CR_LORA_4_6: coderate = 6; break;
            case CR_LORA_4_7: coderate = 7; break;
            case CR_LORA_4_8: coderate = 8; break;
        }

        
        sprintf(cmd, "%s spreading_factor=%u,", cmd, spreading_factor);
        sprintf(cmd, "%s coding_rate=%u,", cmd, coderate);
        sprintf(cmd, "%s implicit_header=%u,", cmd, (pkt_data.no_header ? 1 : 0));
        sprintf(cmd, "%s iq_inverted=%u,", cmd, (pkt_data.invert_pol ? 1 : 0));

    } else if (pkt_data.modulation == MOD_FSK) {
        sprintf(cmd, "%s freq_dev_kHz=%u,", cmd, pkt_data.f_dev);
        sprintf(cmd, "%s baudrate=%u,", cmd, pkt_data.datarate);
    }

    /* check there is enough space in the cmd buffer for the payload */
    if (pkt_data.size >= (sizeof(cmd) - strlen(cmd) - 1)) {
        DEBUG_MSG("ERROR: PAYLOAD TOO BIG FOR SIMULATOR..\n");
        return LGW_HAL_ERROR;
    }

    /* copy payload from user struct to cmd as a hex string */
    sprintf(cmd, "%s data=", cmd);
    for (i = 0; i < pkt_data.size; i++){
        sprintf(cmd, "%s%02X:", cmd, pkt_data.payload[i]);
    }

    /* print created command string into stderr */
    fprintf(stderr, "%s\n", cmd);
...
int lgw_status(uint8_t select, uint8_t *code) {
    ...
    if (select == TX_STATUS) {
        // TODO : implement state machine for Tx status
#ifndef SIMULATOR        
...
int lgw_get_trigcnt(uint32_t* trig_cnt_us) {
    ...
    struct timeval current_unix_time;
    gettimeofday(&current_unix_time, NULL);
    *trig_cnt_us = (uint32_t)(current_unix_time.tv_sec * 1000000UL + current_unix_time.tv_usec);
    return LGW_HAL_SUCCESS;
#endif /* SIMULATOR */
}
