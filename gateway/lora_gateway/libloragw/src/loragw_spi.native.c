...
#define DEBUG_MSG(str)                do {if (debug_flags.reg) fprintf(stderr, str);} while(0)
#define DEBUG_PRINTF(fmt, args...)    do {if (debug_flags.reg) fprintf(stderr,"%s:%d: "fmt, __FUNCTION__, __LINE__, args);} while(0)
...
int lgw_spi_open(void **spi_target_ptr, const char* dev_path, int spi_speed) {
    int *spi_device = NULL;
    int dev;
    int a=0, b=0;
    int i;
