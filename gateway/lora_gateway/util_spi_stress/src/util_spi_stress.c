...
void usage(void) {
    ...
    MSG( " -d <str> enable debug prints for specific comma-separated\n");
    MSG( "          modules (aux,spi,reg,hal,gps,gpio,lbt)\n");
}
...
int main(int argc, char **argv)
{
    ...
    while ((i = getopt (argc, argv, "ht:s:d:S:")) != -1) {
        switch (i) {
            case 'h':
                usage();
                return EXIT_FAILURE;
            ...
            case 't':
                ...
                break;
            case 'S':
                i = sscanf(optarg, "%i", &spi_speed);
                if (i != 1) {
                    MSG("ERROR: invalid SPI speed\n");
                    return EXIT_FAILURE;
                }
                break;
    ...
    /* start SPI link */
    i = lgw_connect(false, DEFAULT_TX_NOTCH_FREQ, spi_path, spi_speed);
