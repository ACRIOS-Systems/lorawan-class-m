...
int main(int argc, char **argv)
{
    ...
    uint8_t g_power = DEFAULT_RF_POWER;
    ...
    while ((i = getopt_long (argc, argv, "hu::f:r:d:", long_options, &option_index)) != -1) {
        switch (i) {
            case 'h':
                ...
                printf(" --power <uint>   Override max Tx power in dBm []\n");
                printf(" -d <str> enable debug prints for specific comma-separated\n");
                printf("          modules (aux,spi,reg,hal,gps,gpio,lbt)\n");
                printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
                return EXIT_SUCCESS;
                break;
            ...
            case 0:
                ...
                else if (strcmp(long_options[option_index].name,"power") == 0) {
                    i = sscanf(optarg, "%u", &arg_u);
                    if ((i != 1) || (arg_u > 27)) {
                        printf("ERROR: argument parsing of --power argument. Use -h to print help\n");
                        return EXIT_FAILURE;
                    }
                    else {
                        g_power = arg_u;
    ...
    txlut.lut[0].rf_power = g_power;
    lgw_txgain_setconf(&txlut);

    /* Start the concentrator */
    i = lgw_start(spi_path, 0);
