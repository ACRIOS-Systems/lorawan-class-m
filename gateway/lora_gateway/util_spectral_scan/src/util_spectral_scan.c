...
int main( int argc, char ** argv )
{
    ...
    while((i = getopt(argc, argv, "hf:n:b:l:o:s:d:")) != -1) {
        switch (i) {
        case 'h':
            printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
    ...
    x = lgw_connect(true, 0, spi_path, 0); /* SPI only, no FPGA reset/configure (for now) */
    if(x != 0) {
    ...
    if (TAKE_N_BITS_FROM((uint8_t)reg_val, FPGA_FEATURE_LBT, 1) == true) {
        ...
        x = lgw_connect(false, LGW_DEFAULT_NOTCH_FREQ, spi_path, 0); /* FPGA reset/configure */
        if(x != 0) {
            printf("ERROR: Failed to connect to FPGA\n");
            return EXIT_FAILURE;
        }
