...
/* Class-M related hf */
static int hf_lorawan_classmInfo_type = -1;
static int hf_lorawan_classmFrame_type = -1;
...
static int hf_lorawan_classm_fixed_type = -1;
static int hf_lorawan_classm_FreeSlots_type = -1;
static int hf_lorawan_classm_NetworkDepth_type = -1;
static int hf_lorawan_classm_deviceEUI_type = -1;
static int hf_lorawan_classm_devNonce_type = -1;
static int hf_lorawan_classm_JoinEUI_type = -1;
static int hf_lorawan_classm_connectToDevice_type = -1;
static int hf_lorawan_classm_TimeSlot_type = -1;
static int hf_lorawan_classm_ChildAddress_type = -1;
static int hf_lorawan_classm_AES_Payload_type = -1;

static int hf_lorawan_classm_length_type = -1;
static int hf_lorawan_classm_deviceAddress_type = -1;
static int hf_lorawan_classm_Sender_type = -1;
static int hf_lorawan_classm_FPort_type = -1;
static int hf_lorawan_classm_FCnt_type = -1;
static int hf_lorawan_classm_payload_type = -1;
static int hf_lorawan_classm_MIC_type = -1;
static int hf_lorawan_classm_message_type = -1;
...
static gint ett_lorawan_classmFrame = -1;
static gint ett_lorawan_classmInfo = -1;
static gint ett_lorawan_classmMessage = -1;
...
#define LORAWAN_CLASSM_MMTYPE_OTAAREGACK					0x84
#define LORAWAN_CLASSM_MMTYPE_OTAAREGREQ					0x85
...
static const value_string lorawan_classm_mmtypenames[] = {
	...
	{ LORAWAN_CLASSM_MMTYPE_OTAAREGACK, "OTAA Registration Acknowledgement" },
	{ LORAWAN_CLASSM_MMTYPE_OTAAREGREQ, "OTAA Registration Request" },
...
uint8_t dissect_lorawan_classm_SENDINGDATA(tvbuff_t *tvb, proto_tree *field_tree, uint8_t current_offset, proto_item *tf, proto_tree *tree){
	proto_item *tf2;
	uint8_t payload_length = 0;
	if (*tvb_get_ptr(tvb, current_offset, -1) == LORAWAN_CLASSM_MMTYPE_MSGFORWARD){
		tf2 = proto_tree_add_item(tf, hf_lorawan_classm_message_type, tvb, current_offset, 11, ENC_NA);
		field_tree = proto_item_add_subtree(tf2, ett_lorawan_classmMessage);
		proto_tree_add_item(field_tree, hf_lorawan_classm_mmtype_type, tvb, current_offset, 1, ENC_LITTLE_ENDIAN);
		current_offset += 1;
		proto_tree_add_item(field_tree, hf_lorawan_classm_deviceAddress_type, tvb, current_offset, 4, ENC_LITTLE_ENDIAN);
		current_offset += 4;
		payload_length = *tvb_get_ptr(tvb, current_offset, -1);
		proto_tree_add_item(field_tree, hf_lorawan_classm_length_type, tvb, current_offset, 1, ENC_LITTLE_ENDIAN);
		current_offset += 1;
		proto_tree_add_item(field_tree, hf_lorawan_classm_Sender_type, tvb, current_offset, 4, ENC_LITTLE_ENDIAN);
		current_offset += 4;
		proto_tree_add_item(field_tree, hf_lorawan_classm_FPort_type, tvb, current_offset, 1, ENC_LITTLE_ENDIAN);
		current_offset += 1;
		proto_tree_add_item(field_tree, hf_lorawan_classm_FCnt_type, tvb, current_offset, 2, ENC_LITTLE_ENDIAN);
		current_offset += 2;
		proto_tree_add_item(field_tree, hf_lorawan_classm_payload_type, tvb, current_offset, payload_length, ENC_NA);
		current_offset += payload_length;
		proto_tree_add_item(field_tree, hf_lorawan_classm_MIC_type, tvb, current_offset, 4, ENC_LITTLE_ENDIAN);
		current_offset += 4;

	}else if (*tvb_get_ptr(tvb, current_offset, -1) == LORAWAN_CLASSM_MMTYPE_MSGTHISNODE){
		tf2 = proto_tree_add_item(tf, hf_lorawan_classm_message_type, tvb, current_offset, 11, ENC_NA);
		field_tree = proto_item_add_subtree(tf2, ett_lorawan_classmMessage);
		proto_tree_add_item(field_tree, hf_lorawan_classm_mmtype_type, tvb, current_offset, 1, ENC_LITTLE_ENDIAN);
		current_offset += 1;
	...
	}else if (*tvb_get_ptr(tvb, current_offset, -1) == LORAWAN_CLASSM_MMTYPE_OTAAREGACK){
		tf2 = proto_tree_add_item(tf, hf_lorawan_classm_message_type, tvb, current_offset, 11, ENC_NA);
		field_tree = proto_item_add_subtree(tf2, ett_lorawan_classmMessage);
		proto_tree_add_item(field_tree, hf_lorawan_classm_mmtype_type, tvb, current_offset, 1, ENC_LITTLE_ENDIAN);
		current_offset += 1;
		proto_tree_add_item(field_tree, hf_lorawan_classm_deviceEUI_type, tvb, current_offset, 8, ENC_LITTLE_ENDIAN);
		current_offset += 8;
		proto_tree_add_item(field_tree, hf_lorawan_classm_TimeSlot_type, tvb, current_offset, 4, ENC_LITTLE_ENDIAN);
		current_offset += 4;
		proto_tree_add_item(field_tree, hf_lorawan_classm_ChildAddress_type, tvb, current_offset, 4, ENC_LITTLE_ENDIAN);
		current_offset += 4;
		proto_tree_add_item(field_tree, hf_lorawan_classm_AES_Payload_type, tvb, current_offset, 16, ENC_LITTLE_ENDIAN);
		current_offset += 16;
		
	}else if (*tvb_get_ptr(tvb, current_offset, -1) == LORAWAN_CLASSM_MMTYPE_OTAAREGREQ){
		tf2 = proto_tree_add_item(tf, hf_lorawan_classm_message_type, tvb, current_offset, 11, ENC_NA);
		field_tree = proto_item_add_subtree(tf2, ett_lorawan_classmMessage);
		proto_tree_add_item(field_tree, hf_lorawan_classm_mmtype_type, tvb, current_offset, 1, ENC_LITTLE_ENDIAN);
		current_offset += 1;	
		proto_tree_add_item(field_tree, hf_lorawan_classm_connectToDevice_type, tvb, current_offset, 4, ENC_LITTLE_ENDIAN);
		current_offset += 4;
		proto_tree_add_item(field_tree, hf_lorawan_classm_devNonce_type, tvb, current_offset, 2, ENC_LITTLE_ENDIAN);
		current_offset += 2;
		proto_tree_add_item(field_tree, hf_lorawan_classm_JoinEUI_type, tvb, current_offset, 8, ENC_LITTLE_ENDIAN);
		current_offset += 8;
		proto_tree_add_item(field_tree, hf_lorawan_classm_deviceEUI_type, tvb, current_offset, 8, ENC_LITTLE_ENDIAN);
		current_offset += 8;
	}else{
		printf("Unknown MESSAGE, length remaining: %d\n", tvb_captured_length_remaining(tvb, current_offset));
		return current_offset;
	}
	printf("Length remaining MESSAGE: %d\n", tvb_captured_length_remaining(tvb, current_offset));
	if (tvb_captured_length_remaining(tvb, current_offset)>4){// 4 == MIC length
		current_offset = dissect_lorawan_classm_SENDINGDATA(tvb, field_tree, current_offset, tf, tree);
	}
	return current_offset;
}

void dissect_lorawan_classm_MMTYPE(tvbuff_t *tvb, proto_tree *field_tree, uint8_t current_offset, proto_item *tf, proto_tree *tree){

	tf = proto_tree_add_item(tree, hf_lorawan_classmFrame_type, tvb, current_offset, 11, ENC_NA);
	field_tree = proto_item_add_subtree(tf, ett_lorawan_classmFrame);

	if (*tvb_get_ptr(tvb, current_offset, -1) == LORAWAN_CLASSM_MMTYPE_CRWBEACON){
		proto_tree_add_item(field_tree, hf_lorawan_classm_mmtype_type, tvb, current_offset, 1, ENC_LITTLE_ENDIAN);
		current_offset += 1;
		proto_tree_add_item(field_tree, hf_lorawan_classm_FreeSlots_type, tvb, current_offset, 1, ENC_LITTLE_ENDIAN);
		current_offset += 1;
		proto_tree_add_item(field_tree, hf_lorawan_classm_NetworkDepth_type, tvb, current_offset, 1, ENC_LITTLE_ENDIAN);
		current_offset += 1;

	}else if (*tvb_get_ptr(tvb, current_offset, -1) == LORAWAN_CLASSM_MMTYPE_SYNCBEACON){
		proto_tree_add_item(field_tree, hf_lorawan_classm_mmtype_type, tvb, current_offset, 1, ENC_LITTLE_ENDIAN);
		current_offset += 1;
	...
	}else if (*tvb_get_ptr(tvb, current_offset, -1) == LORAWAN_CLASSM_MMTYPE_SENDINGDATA){
		proto_tree_add_item(field_tree, hf_lorawan_classm_mmtype_type, tvb, current_offset, 1, ENC_LITTLE_ENDIAN);
		current_offset += 1;
		proto_tree_add_item(field_tree, hf_lorawan_classm_length_type, tvb, current_offset, 1, ENC_LITTLE_ENDIAN);
		current_offset += 1;
		...
		printf("Unknown FRAME, length remaining: %d\n", tvb_captured_length_remaining(tvb, current_offset));
		return;
	}
	printf("Length remaining FRAME: %d\n", tvb_captured_length_remaining(tvb, current_offset));
	if (tvb_captured_length_remaining(tvb, current_offset)>0){
		dissect_lorawan_classm_MMTYPE(tvb, field_tree, current_offset, tf, tree);
	}
}
...
dissect_lorawan_classm(tvbuff_t *tvb, packet_info *pinfo _U_, proto_tree *tree _U_)
{
	...
	proto_tree *field_tree;
	gint32 current_offset = 1;
	tf = proto_tree_add_item(tree, hf_lorawan_classmInfo_type, tvb, current_offset, 11, ENC_NA);
	field_tree = proto_item_add_subtree(tf, ett_lorawan_classmInfo);
	...
	current_offset += 2;
	printf("\nMMTYPE: %02X\n", *tvb_get_ptr(tvb, current_offset, -1));
	dissect_lorawan_classm_MMTYPE(tvb, field_tree, current_offset, tf, tree);
	return tvb_captured_length(tvb);
}
...
proto_register_lorawan(void)
{
	...
	{ &hf_lorawan_classmInfo_type,
		{ "Class-M Info", "lorawan.classmInfo",
		FT_NONE, BASE_NONE,
		NULL, 0x0,
		NULL, HFILL }
	},
	{ &hf_lorawan_classmFrame_type,
		{ "Class-M Frame", "lorawan.classmFrame",
		FT_NONE, BASE_NONE,
		NULL, 0x0,
		NULL, HFILL }
	},
	...
	{ &hf_lorawan_classm_fixed_type,
		{ "Class-M Fixed field", "lorawan.classm.fixed",
		FT_UINT32, BASE_DEC,
		NULL, 0x0,
		"[MMType] Class-M Fixed field", HFILL }
	},
	{ &hf_lorawan_classm_FreeSlots_type,
		{ "Class-M Free Slots field", "lorawan.classm.FreeSlots",
		FT_UINT8, BASE_DEC,
		NULL, 0x0,
		"[MMType] Class-M Free Slots field", HFILL }
	},
	{ &hf_lorawan_classm_NetworkDepth_type,
		{ "Class-M Network Depth field", "lorawan.classm.NetworkDepth",
		FT_UINT8, BASE_DEC,
		NULL, 0x0,
		"[MMType] Class-M Network Depth field", HFILL }
	},
	{ &hf_lorawan_classm_deviceEUI_type,
		{ "Class-M Device EUI", "lorawan.classm.DeviceEUI",
		FT_UINT64, BASE_HEX,
		NULL, 0x0,
		"[MMType] Class-M Device EUI", HFILL }
	},
	{ &hf_lorawan_classm_devNonce_type,
		{ "Class-M devNonce", "lorawan.classm.devNonce",
		FT_UINT16, BASE_DEC,
		NULL, 0x0,
		"[MMType] Class-M devNonce", HFILL }
	},
	{ &hf_lorawan_classm_JoinEUI_type,
		{ "Class-M JoinEUI", "lorawan.classm.JoinEUI",
		FT_UINT64, BASE_HEX,
		NULL, 0x0,
		"[MMType] Class-M JoinEUI", HFILL }
	},
	{ &hf_lorawan_classm_connectToDevice_type,
		{ "Class-M Connect to device", "lorawan.classm.connectToDevice",
		FT_UINT32, BASE_HEX,
		NULL, 0x0,
		"[MMType] Class-M Connect to device", HFILL }
	},
	{ &hf_lorawan_classm_TimeSlot_type,
		{ "Class-M Time Slot", "lorawan.classm.TimeSlot",
		FT_UINT32, BASE_DEC,
		NULL, 0x0,
		"[MMType] Class-M Time Slot", HFILL }
	},
	{ &hf_lorawan_classm_ChildAddress_type,
		{ "Class-M Child Address", "lorawan.classm.childAddress",
		FT_UINT32, BASE_HEX,
		NULL, 0x0,
		"[MMType] Class-M Child Address", HFILL }
	},
	{ &hf_lorawan_classm_AES_Payload_type,
		{ "Class-M Payload", "lorawan.classm.AES_Payload",
		FT_BYTES, BASE_NONE,
		NULL, 0x0,
		"[MMType] Class-M Payload", HFILL }
	},
	{ &hf_lorawan_classm_length_type,
		{ "Class-M Length", "lorawan.classm.length",
		FT_UINT8, BASE_DEC,
		NULL, 0x0,
		"[MMType] Class-M Length", HFILL }
	},
	{ &hf_lorawan_classm_deviceAddress_type,
		{ "Class-M Receiver Address", "lorawan.classm.deviceAddress",
		FT_UINT32, BASE_DEC,
		NULL, 0x0,
		"[MMType] Class-M Receiver Address", HFILL }
	},
	{ &hf_lorawan_classm_Sender_type,
		{ "Class-M Sender Address", "lorawan.classm.Sender",
		FT_UINT32, BASE_HEX,
		NULL, 0x0,
		"[MMType] Class-M Sender Address", HFILL }
	},
	{ &hf_lorawan_classm_FPort_type,
		{ "Class-M FPort", "lorawan.classm.FPort",
		FT_UINT8, BASE_DEC,
		NULL, 0x0,
		"[MMType] Class-M FPort", HFILL }
	},
	{ &hf_lorawan_classm_FCnt_type,
		{ "Class-M FCnt", "lorawan.classm.FCnt",
		FT_UINT16, BASE_DEC,
		NULL, 0x0,
		"[MMType] Class-M FCnt", HFILL }
	},
	{ &hf_lorawan_classm_payload_type,
		{ "Class-M Message Payload", "lorawan.classm.payload",
		FT_BYTES, BASE_NONE,
		NULL, 0x0,
		"[MMType] Class-M Message Payload", HFILL }
	},
	{ &hf_lorawan_classm_MIC_type,
		{ "Class-M MIC", "lorawan.classm.MIC",
		FT_UINT32, BASE_DEC,
		NULL, 0x0,
		"[MMType] Class-M MIC", HFILL }
	},
	{ &hf_lorawan_classm_message_type,
		{ "Class-M Message", "lorawan.classm.message",
		FT_NONE, BASE_NONE,
		NULL, 0x0,
		"[MMType] Class-M Message", HFILL }
	},
	};
	...
	static gint *ett[] = {
		...
		&ett_lorawan_classmFrame,
		&ett_lorawan_classmInfo,
		&ett_lorawan_classmMessage
	};
